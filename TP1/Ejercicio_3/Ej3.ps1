...
.DESCRIPTION
Recibe un txt con los datos para hacer resolver el sistema de ecuaciones y lo guarda en otro txt de salida.
...
foreach($obj in $contenido) #foreach para recorrer el $contenido, es un vector de lineas de texto
{
    if($i) #entra solo para agarrar la primera linea osea n.
    ...
    else
        {
            ...
            {
                write "La fila $ContFilas,[$linea]no respeta las dimensiones del n ($n)"
                exit;
            
            }
            $array.Add($linea) >$null     
        }
}
    if($ContFilas -ne $n)
    {
...
for( $j=$n-1; $j -ge 0; $j-- ){
    ...
    try{$x[$j]=[double]($array[$j][$n] / $array[$j][$j])}
    catch{
    cls
    Write-Host error hay division x cero en un elemento del sistema
    exit}
